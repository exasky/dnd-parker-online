...
import { CharacterCreatorComponent } from "../character/character-creator.component";
import { CharacterItemDisplayerComponent } from "../character-item-displayer.component";
import { AdventureCreatorComponent } from "../adventure/adventure-creator.component";
    ...
    if (id !== null) {
      this.campaignService.getCampaign(id).subscribe((campaign) => {
        ...
        this.adventureService.getCharacterTemplates().subscribe((value) => {
          this.characterTemplates = value.filter(
            (ct) => this.campaign.characters.findIndex((char) => char.name === ct.name) === -1,
          );
        });
      });
    } else {
  ...
  deleteCampaign() {
    const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      ...
      data: {
        ...
        confirmMessage: "Are ou sure you want to delete " + this.campaign.name + "?",
      },
    });
