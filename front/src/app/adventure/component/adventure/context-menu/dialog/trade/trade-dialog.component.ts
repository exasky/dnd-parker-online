...
export class TradeDialogComponent implements OnInit, OnDestroy {
  ...
  selectedTo: { characterEquipment: CharacterEquipment, isEquipment } = {characterEquipment: null, isEquipment: false};
  ...
  isClickAuthorized(isFrom: boolean): boolean {
    return this.authService.currentUserValue.characters
      .some(userChar => userChar.id === (isFrom ? this.data.trade.from.id : this.data.trade.to.id));
  }
